...
def getFile(driver, list_prodi_ujian, filters):
    ...
    for prodi_selected in list_prodi_ujian:
        ...
        checkDirProcess(prodi_selected)
    ...
    while True:
        try:
            ...
            if os.path.exists('absensi/' + prodi + '/' + filename+config.file_type):
                removeFileProcess('absensi/' + prodi + '/' +
            ...
            if filename.endswith(config.file_type) and filename.startswith(filters['tahun']+'-'+convertUjianByNumber(filters['jenis'])+'-'+filters['program']):
                ...
                if email_dosen == 'NULL':
                    ...
                    file_baru = nama_baru[0]+'-'+nama_baru[1]+'-'+nama_baru[2]+'-' + \
                        nama_baru[3]+'-'+nama_baru[4]+'-' + \
                        nama_baru[5]+config.file_type
...
def sendEmailProcess(file):
    filters = {
        'tipe': 'send email',
        'file': file
    }
    subprocess.Popen(
        ["python", "siap_jadwal_process.py", str(filters), ])
    
def sendEmailExec(file):
    sendEmail(file)
...
def removeFileProcess(namafile):
    filters = {
        'tipe': 'remove file',
        'namafile': namafile
    }
    subprocess.Popen(["python", "siap_jadwal_process.py", str(filters), ])

def removeFileExec(namafile):
    os.remove(namafile)


def checkSubProcess():
    if sys.argv != ['main.py']:
        filters = json.loads(str(sys.argv[1:])[2:-2].replace("\'", "\""))
        # print(type(filters))
        # print(filters)
        if filters['tipe'] == 'check dirs' or filters['tipe'] == 'check dir':
            checkDirsExec(filters['path'], filters['path'])
        elif filters['tipe'] == 'remove file':
            removeFileExec(filters['namafile'])
        elif filters['tipe'] == 'send email':
            sendEmailExec(filters['file'])
